...
\caption[Comparison of fry body ratio by condition]{\footnotesize{Comparison of fry body
      ratio by condition. \\ The body ratio of each fry
...
\textit{SD} = 0.11), \textit{t}(degrees freedom) = 0.70, \textit{p} = 0.25. There was also no
significant difference between the length alone (\textit{M} separated = 2.31 cm,
\textit{SD} = ;
\textit{M} unseparated = 2.49 cm, \textit{SD} = ), \textit{t}(degrees freedom) =
,
\textit{p} = 0.51; nor the weight
alone(\textit{M} separated = 0.52 g, \textit{SD} = ; \textit{M} unseparated =
0.60 g, \textit{SD} = ) \textit{t}(degrees freedom) = ,\textit{p}
= 0.65.
...
\caption[Comparison of fry body ratio by brood size]{\footnotesize{Comparison of fry body
      ratio by brood size. \\ Fry from broods containing only one fish
      (\textit{M} = , \textit{SD} = ) had
      a significantly higher
body ratio compared to broods with two (\textit{M} = , \textit{SD} = ) or three
(\textit{M} = , \textit{SD} = )
fry, \textit{F}(2, 15) = , \textit{p} < 0.05.}}
\label{subd}
\end{center} 
\end{figure}

A one-way ANOVA was used to compare size based on number of fry in the brood. Broods in which there were only one fish were significantly greater in
body ratio compared to broods with two (\textit{M} = , \textit{SD} = ) or three
(\textit{M} = , \textit{SD} = )
fry, \textit{F}(2, 15) = , \textit{p} < 0.05. Additionally, broods with
only one fry (\textit{M} = , \textit{SD} = ) were longer than broods with two
(\textit{M} = , \textit{SD} = ) or three (\textit{M} = , \textit{SD} = ) fry,
\textit{F}(2, 15) = , (\textit{p} < 0.05). Further, fry from single broods
(\textit{M} = , \textit{SD} = ) weighed more than fry from broods of two
(\textit{M} = , \textit{SD} = ) or three (\textit{M} = , \textit{SD} = ) fry,
\textit{F}(2, 15) = ,
\textit{p} < 0.01. A post-hoc Tukey .....
...
(\textit{Mdn} = ) compared to those unseparated from their mothers (\textit{Mdn}
= ), \textit{U} = , \textit{p} < 0.05, \textit{r} = . There was no significant difference between conditions in the
time spent at the top (\textit{Mdn} separated = , \textit{Mdn} unseparated =
; \textit{U} = , \textit{p} = 0.22, \textit{r} = ) and behind the shelter (\textit{Mdn} separated = ,
\textit{Mdn} unseparated = ; \textit{U} = , \textit{p} = 0.08, \textit{r} = ). 
...
groups]{\footnotesize{Comparison of tank aggression between groups.\\ Fry aggression was measured
  ...
  there was no difference between fry separated from their mothers (\textit{Mdn}
  = ) and fry kept with their mothers (\textit{Mdn} = ), \textit{U} = ,
  \textit{p} = }, \textit{U} = .}
\label{subd}
\end{center} 
\end{figure}
...
(\textit{Mdn} = ) and unseparated (\textit{Mdn} = ) groups, \textit{U}
= , \textit{p} = , \textit{r} = .
...
unseparated = 0.92, \textit{t} = 0.78, \textit{p} = 0.47; GR1b \textit{M} separated = 0.86, \textit{M}
unseparated = 1.00, \textit{t} = -1.44 \textit{p} = 0.17; GR2 \textit{M} separated = 1.14, \textit{M}
unseparated = 1.12, \textit{t} = 0.26, \textit{p} = 0.81). There was a statistically significant
